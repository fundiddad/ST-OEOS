...
```
src/
├── oeos-st-extension/             # Complete OEOS plugin (core files + build artifacts)
...
│   │
│   ├── js/                        # Vue build artifacts (auto-generated by deploy.js)
│   │   ├── app.js                 # Vue application main file
│   │   └── app.js.map             # Source map
│   ├── fonts/                     # Font files (build artifacts)
│   ├── crsp/                      # CRSP module (build artifacts)
│   ├── index.html                 # Vue app entry (build artifacts)
│   ├── bg-texture.png             # Background texture (build artifacts)
│   ├── favicon.ico                # Icon (build artifacts)
│   └── loading.gif                # Loading animation (build artifacts)
│
├── openeos-master/                # openOEOS player (Vue 2 project)
│   ├── src/                       # Vue source code
│   ├── public/                    # Static resources
│   ├── dist/                      # Build output (temp dir, copied to oeos-st-extension/)
│   ├── package.json               # Dependencies and build scripts
│   ├── vue.config.js              # Webpack configuration
│   ├── deploy.js                  # Deployment script (new flow: merge to oeos-st-extension, then copy to ST)
│   └── README.md                  # openOEOS player documentation
│
├── SillyTavern-release/           # SillyTavern installation directory (not tracked)
│   └── public/scripts/extensions/third-party/
│       └── oeos-st-extension/     # Final deployment location (copied from src/oeos-st-extension/)
│
...
2. **Build Project**
   ...
   - Execute `deploy.js` script:
     1. Merge `dist/` build artifacts into `src/oeos-st-extension/` (making it a complete plugin)
     2. Copy the complete `src/oeos-st-extension/` to SillyTavern directory

   Final output locations:
   ```
   src/oeos-st-extension/                                                      # Complete plugin (with build artifacts)
   src/SillyTavern-release/public/scripts/extensions/third-party/oeos-st-extension/  # SillyTavern deployment location
   ```


### Build Notes

- **Plugin source directory**: `src/oeos-st-extension/` is now the complete plugin folder (core files + build artifacts)
- **No need to manually modify** `SillyTavern-release/public/scripts/extensions/third-party/oeos-st-extension/` directory
- **Modify plugin core files**: Edit `.js` files in `src/oeos-st-extension/`
- **Modify Vue player code**: Edit in `src/openeos-master/src/`
- **Each build**:
  1. First merges build artifacts into `src/oeos-st-extension/`
  2. Then copies the complete plugin to SillyTavern extension directory
- **Git tracking**: Build artifacts in `src/oeos-st-extension/` are also committed to Git
