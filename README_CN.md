...
```
src/
├── oeos-st-extension/             # 完整的 OEOS 插件（包含核心文件 + 构建产物）
...
│   ├── README.md                  # 插件文档
│   │
│   ├── js/                        # Vue 构建产物（由 deploy.js 自动生成）
│   │   ├── app.js                 # Vue 应用主文件
│   │   └── app.js.map             # Source map
│   ├── fonts/                     # 字体文件（构建产物）
│   ├── crsp/                      # CRSP 模块（构建产物）
│   ├── index.html                 # Vue 应用入口（构建产物）
│   ├── bg-texture.png             # 背景纹理（构建产物）
│   ├── favicon.ico                # 图标（构建产物）
│   └── loading.gif                # 加载动画（构建产物）
│
├── openeos-master/                # openOEOS 播放器（Vue 2 项目）
│   ├── src/                       # Vue 源代码
│   ├── public/                    # 静态资源
│   ├── dist/                      # 构建输出（临时目录，会被复制到 oeos-st-extension/）
│   ├── package.json               # 依赖和构建脚本
│   ├── vue.config.js              # Webpack 配置
│   ├── deploy.js                  # 部署脚本（新流程：先合并到 oeos-st-extension，再复制到 ST）
│   └── README.md                  # openOEOS 播放器文档
│
├── SillyTavern-release/           # SillyTavern 安装目录（不跟踪）
│   └── public/scripts/extensions/third-party/
│       └── oeos-st-extension/     # 最终部署位置（从 src/oeos-st-extension/ 复制）
│
├── package.json                   # 根项目依赖
├── README.md                      # 英文文档
...
2. **构建项目**
   ...
   - 执行 `deploy.js` 脚本：
     1. 将 `dist/` 构建产物合并到 `src/oeos-st-extension/`（使其成为完整插件）
     2. 将完整的 `src/oeos-st-extension/` 复制到 SillyTavern 目录

   最终输出位置：
   ```
   src/oeos-st-extension/                                                      # 完整插件（包含构建产物）
   src/SillyTavern-release/public/scripts/extensions/third-party/oeos-st-extension/  # SillyTavern 部署位置
   ```


### 构建说明

- **插件源目录**：`src/oeos-st-extension/` 现在是完整的插件文件夹（包含核心文件 + 构建产物）
- **不需要手动修改** `SillyTavern-release/public/scripts/extensions/third-party/oeos-st-extension/` 目录
- **修改插件核心文件**：在 `src/oeos-st-extension/` 中修改 `.js` 文件
- **修改 Vue 播放器代码**：在 `src/openeos-master/src/` 中修改
- **每次构建**：
  1. 先将构建产物合并到 `src/oeos-st-extension/`
  2. 再将完整插件复制到 SillyTavern 扩展目录
- **Git 跟踪**：`src/oeos-st-extension/` 中的构建产物也会被提交到 Git
