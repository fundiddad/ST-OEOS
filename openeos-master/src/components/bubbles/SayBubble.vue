<template>
  ...
  >
    <global-events
      v-if="active && (skip || (isDebug && !noPause))"
...
export default {
  ...
  computed: {
    ...
    showButton() {
      return this.active && (this.isPause || (!this.noPause && this.skip))
    },
    isPause() {
    ...
    skip() {
      return this.value.skip || this.noPause || this.value.isAuto
    },
    onContinue() {
  ...
  methods: {
    ...
    onClick(event) {
      if (
        this.active &&
        (this.skip || this.isPause || (this.isDebug && !this.noPause))
      ) {
